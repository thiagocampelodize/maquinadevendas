    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top', 'bottom']}>
      <View className="px-4 pt-4">
        <SubmenuHeaderCard
          ...
          right={<Text className="text-xs text-text-muted">Não lidas: {unreadCount}</Text>}
        />
      </View>
      ...
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ gap: 10, paddingTop: 16, paddingBottom: 24 }}>
        {loading ? <Text className="text-text-muted">Carregando...</Text> : null}

        {!loading && items.length === 0 ? (
          <View className="rounded-xl border border-border bg-card p-4">
            <Text className="text-text-muted">Nenhuma notificação encontrada.</Text>
          </View>
        ) : null}
        ...
        {items.map((notification) => (
          <Pressable
            key={notification.id}
            className={`rounded-xl border p-4 ${notification.read ? 'border-border bg-card' : 'border-[#FF6B35] bg-[#FF6B3515]'}`}
          ...
          >
            ...
            <Text className="text-sm text-text-secondary">{notification.message || 'Sem mensagem adicional.'}</Text>
          </Pressable>
        ))}
      </ScrollView>
    </SafeAreaView>
