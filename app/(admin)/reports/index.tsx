        ...
        <View className="rounded-2xl border border-border bg-surface p-4">
          ...
          {loading ? (
            <View className="py-8">
              ...
              <Text className="mt-2 text-center text-sm text-text-muted">Gerando visão de relatórios...</Text>
            </View>
          ) : (
            <View className="mt-3 gap-2">
              {items.map((report) => (
                <View key={report.id} className="rounded-xl border border-border bg-card p-3">
                  <View className="flex-row items-center gap-2">
                    <FileText size={14} color="#FF6B35" />
                    <Text className="flex-1 text-sm font-semibold text-white">{report.title}</Text>
                  </View>
                  <Text className="mt-1 text-xs text-text-muted">Gerado em {report.generatedAt}</Text>
                  <Text className="text-xs text-text-muted">Formato: {report.format}</Text>
                </View>
              ))}
            </View>
          )}
