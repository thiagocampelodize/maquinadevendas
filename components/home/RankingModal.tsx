        ...
        >
          ...
          <View className="flex-row gap-2 p-4">
            <Pressable
              ...
              className={`flex-1 rounded-md px-3 py-2 ${rankingView === 'value' ? 'bg-[#FF6B35]' : 'bg-card-elevated'}`}
            >
              <Text className="text-center text-sm text-white">Por Valor</Text>
            </Pressable>
            <Pressable
              onPress={() => setRankingView('percentage')}
              className={`flex-1 rounded-md px-3 py-2 ${rankingView === 'percentage' ? 'bg-[#FF6B35]' : 'bg-card-elevated'}`}
            >
          ...
          <ScrollView className="flex-1 px-4" contentContainerStyle={{ gap: 10, paddingBottom: 24 }}>
            {currentRanking.length === 0 ? (
              <View className="rounded-xl border border-border bg-card p-4">
                <Text className="text-center text-sm text-text-muted">Nenhum vendedor encontrado.</Text>
              </View>
            ) : null}
            ...
            {currentRanking.map((seller, index) => {
              ...
              return (
                <View key={seller.id} className="rounded-xl border border-border bg-card p-3">
...
function Cell({
  ...
  return (
    <View className={`min-w-[90px] rounded-md p-2 ${highlight ? 'bg-[#FF6B351A]' : 'bg-card-elevated'}`}>
